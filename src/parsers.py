...
def parser_shr(tokens_list,ast_class,table:ParserTable,attr_decoder):
        ...
        while True:
            ...
            if action==table.Action.SHIFT:
                ...
                to_remov=tag[1]    
                state_stack=state_stack[:-to_remov]
                n_last_state=state_stack[-1]
                s_to_reduce= symbol_stack[-to_remov:]
                symbol_stack = symbol_stack[:-to_remov]
                ...
                raise ValueError(f"Parsing Error in {token}")
